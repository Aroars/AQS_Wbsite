...
export interface RoiInputs {
  ...
  overtimeHoursPerWeek: number; // Weekly overtime hours per worker (default 0)
  overtimeMultiplier: number; // Overtime rate multiplier (default 1.5x)
...
export const DEFAULT_INPUTS: RoiInputs = {
  ...
  laborUnitsReduced: 2,
  overtimeHoursPerWeek: 0, // No overtime by default
  overtimeMultiplier: 1.5, // Industry standard time-and-a-half
...
export type ModifiedFields = Set<keyof RoiInputs>;

// Helper to check if a field is at its default value
export const isDefaultValue = <K extends keyof RoiInputs>(
  key: K,
  value: RoiInputs[K],
  modifiedFields: ModifiedFields
): boolean => {
  // If the field has been explicitly modified, it's not a default
  if (modifiedFields.has(key)) {
    return false;
  }
  // Otherwise, compare to default value
  return JSON.stringify(value) === JSON.stringify(DEFAULT_INPUTS[key]);
};
