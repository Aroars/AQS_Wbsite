...
export const toolPages: ToolPage[] = [
    ...
    },
    {
        ...
        intro: [
            'A plant quotes throughput in the units it thinks in — 15,000 lb/hr, 60,000 lb/day over two shifts, 600 kg/hr — and a conveyor is sized in packages per minute, belt speed, and pounds per foot of belt. This calculator converts between them as you type. Pick Packages or Bulk, then enter what you know: any two of line throughput, package weight, and packages per minute give the third; product length and gap give the belt speed that carries that rate, or enter belt speed directly. The result is the belt load in lb/ft the belt pull calculator needs, with pitch and products per foot alongside — and belt load is a field of its own, so a known lb/ft backs out the speed or the gap instead.',
            'Every field solves in both directions. Overwrite a derived number and the calculator releases the oldest value you entered and re-derives it, so the numbers on the card always agree; lock the two or three knowns the job starts from and only the rest ever move. Bulk mode does the same for loose product: density, bed depth, and belt width give the lb/ft a bed presents, which at belt speed is the throughput, or a demanded rate and speed give the depth the bed needs. A bulk conveyor feeding a bagger can start from the other end: bags per minute × bag weight — or settled density × bag volume — backs out the lb/hr the belt must deliver. A packager headroom check compares the required packages per minute with the machine\'s nameplate rate, and a maximum bed depth checks the bed against a side guard or flight height.',
        ],
        howItWorks: [
            ...
            'Bulk: lb/ft = density × (bed depth ÷ 12) × ((belt width − 2 × edge margin) ÷ 12); throughput = lb/ft × belt speed. Feeding a bagger: throughput (lb/min) = bags/min × bag weight, where bag weight can be settled density × bag volume × fill.',
        ...
        faq: [
            ...
            { q: 'How do I size a bulk conveyor when I only know the bagger rate?', a: 'Switch to Bulk and enter the bags per minute and the weight per bag in the downstream packager section — or the settled density and bag dimensions in the fill box. The line throughput backs out of them: 20 bags/min at 2.5 lb is 50 lb/min or 3,000 lb/hr. Add the loose density, belt width, and belt speed and the calculator gives the bed depth the belt must carry.' },
        ],
        product: { href: '/solutions/conveyors', label: 'Sanitary conveyors by AQS' },
